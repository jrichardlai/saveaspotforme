SaveASpotForMe = {
  ...
  validateOrder: function (e) {
    ...
    if (!SaveASpotForMe.validateDeliveryHours()) {
      if (e) { e.preventDefault(); }
  ...
  refreshTask: function () {
    $.ajax({
      url: SaveASpotForMe.taskUrl,
      data: {'access_token': SaveASpotForMe.token},
      dataType: 'jsonp',
      success: function (data, status) {
        ...
        if (data.state == 'assigned') {
          ...
          setTimeout(SaveASpotForMe.refreshTask, 5000);
        }
  ...
  OrderList: {
    ...
    newLineItem: function (value) {
      ...
      if(num == 1 || prev && prev.replace(/ /g,'').length > 0){
        ...
        newItem = $('<li class="listItem" id="orderItem'+num+'"><label class="itemLabel">'+num+'</label><input type="text" class="itemInput" name="input'+num+'" value="'+value+'" placeholder="'+randomItem+'" autocomplete="off" onkeyup="SaveASpotForMe.OrderList.saveLineItem(this)" /><div class="kill"></div></li>');
